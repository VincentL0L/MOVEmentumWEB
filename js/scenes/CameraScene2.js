export default class CameraScene2 extends Phaser.Scene {
    constructor() {
        super("CameraScene2");
        this.keypoints = null;
        this.starRadius = 80;
    ...
    update(time, delta) {
        ...
        if (this.keypoints) {
            ...
            // Jumping Jack Detection (hands below waist → above head)
            const leftWrist = this.keypoints[15];
            const rightWrist = this.keypoints[16];
            const leftHip = this.keypoints[23];
            const rightHip = this.keypoints[24];

            if (leftWrist && rightWrist && leftHip && rightHip) {
                const avgHipY = (leftHip.y + rightHip.y) / 2;
                const handsAboveHead = leftWrist.y < 0.3 && rightWrist.y < 0.3;
                const handsBelowWaist = leftWrist.y > avgHipY + 0.1 && rightWrist.y > avgHipY + 0.1;

                if (!this.jackPhase) {
                    this.jackPhase = "down";
                }

                if (this.jackPhase === "down" && handsAboveHead) {
                    this.jackPhase = "up";
                } else if (this.jackPhase === "up" && handsBelowWaist) {
                    this.jackPhase = "down";
                    this.score++;
            ...
            if (this.poseGraphics) {
                ...
                this.keypoints.forEach((kp, i) => {
                    if (kp && kp.visibility > 0.1) {
                        const x = width - kp.x * width;
                        const y = kp.y * height;
                        this.add.text(x + 5, y - 5, i.toString(), { fontSize: '12px', fill: '#ffffff' }).setDepth(1004);
                    }
                });
            }
