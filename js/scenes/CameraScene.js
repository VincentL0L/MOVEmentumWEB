...
export default class CameraScene extends Phaser.Scene {
    ...
    update(time, delta) {
        ...
        if (this.video.readyState === this.video.HAVE_ENOUGH_DATA) {
            ...
            this.videoCtx.drawImage(this.video, -this.scale.width, 0, this.scale.width, this.scale.height);
        ...
        if (this.keypoints) {
            ...
            if (ls && rs) {
                const cx = (ls && rs) ? (width - ((ls.x + rs.x) / 2) * width) : 0;
                const cy = (ls && rs) ? ((ls.y + rs.y) / 2) * height : 0;
            ...
            const lp = { x: lw.x * width, y: lw.y * height };
            const rp = { x: rw.x * width, y: rw.y * height };
